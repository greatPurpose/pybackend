...
class ModelTests(TestCase):
    ...
    def test_create_new_superuser(self):
        ...
        user = get_user_model().objects.create_superuser("4086432477", "test123")
    ...
    def test_tag_str(self):
        ...
        tag = models.Tag.objects.create(user=sample_user(), name="Insurance")

        self.assertEqual(str(tag), tag.name)

    def test_coverage_str(self):
        """Test the coverage string representation"""
        coverage = models.Coverage.objects.create(
            user=sample_user(), name="this coverage"
        )

        self.assertEqual(str(coverage), coverage.name)

    def test_product_str(self):
        """Test the product string representation"""
        product = models.Product.objects.create(
            user=sample_user(), title="auto", price=20.00
        )
